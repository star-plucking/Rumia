...
#define SPI_TRANSMIT(buf, size)         HAL_SPI_Transmit_DMA(&hspi2, (buf), (size));
...
/**
 * @brief  渐变彩色呼吸效果，亮度逐渐从0到最高，再逐渐回到0
 * 
 * @param  speed: 变化速度（毫秒）
 */
void WS2812_BreathingRainbow(uint16_t speed)
{
    uint16_t i, j;

    // 渐变过程分成多个步长
    for (j = 0; j < 256; j++)  // 颜色变化循环
    {
        uint8_t brightness = (uint8_t)(j); // 从0到255
        for (i = 0; i < WS2812_NUM; i++)
        {
            uint32_t color = WS2812_Wheel((i + j) & 255);
            // 应用亮度到颜色
            uint32_t finalColor = ((color & 0xFF0000) * brightness / 255) & 0xFF0000 |  // Red
                                  ((color & 0x00FF00) * brightness / 255) & 0x00FF00 |  // Green
                                  ((color & 0x0000FF) * brightness / 255) & 0x0000FF;  // Blue
            WS2812_OneSet(i, finalColor);
        }
        WS2812_Show();
        delay_ms(speed);
    }
    ...
    // 减少亮度阶段
    for (j = 255; j > 0; j--)  // 颜色变化循环
    {
        uint8_t brightness = (uint8_t)(j); // 从255到0
        for (i = 0; i < WS2812_NUM; i++)
        {
            uint32_t color = WS2812_Wheel((i + j) & 255);
            // 应用亮度到颜色
